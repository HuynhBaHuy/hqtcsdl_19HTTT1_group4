...
CREATE PROCEDURE spUpdateOrderStatusForDriver_T1_fixed @matx varchar(20), @madh varchar(20), @ttdh nvarchar(50)
AS
BEGIN TRAN
		...
		BEGIN
			...
			ELSE
				BEGIN
					...
					IF (SELECT TinhTrangDH FROM DON_HANG WITH(XLOCK) WHERE MaDH = @madh) = 'Đã hoàn trả hàng'
